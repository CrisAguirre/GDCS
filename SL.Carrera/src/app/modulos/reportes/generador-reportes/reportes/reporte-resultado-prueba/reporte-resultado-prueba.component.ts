...
import { RptIdConvocatoriaModel } from '@app/compartido/modelos/reportes/rpt-id-convocatoria-model';
...
@Component({
  selector: 'app-reporte-resultado-prueba',
  templateUrl: './reporte-resultado-prueba.component.html',
  styleUrls: ['./reporte-resultado-prueba.component.scss']
})
export class ReporteResultadoPruebaComponent extends BaseController implements OnInit, AfterViewChecked {
  ...
  public async loadConvocatoriasByEmpresa(empresaEvent: any) {
    ...
    const lstConvocatoriaAux = (await this.convocatoryServices.getTodosConvocatoriasByEmpresa(empresaEvent.value).toPromise() as any).datos as Convocatoria[];
  ...
  public formValid(val) {
    if (val && this.form.valid) {
      const dataReport: RptIdConvocatoriaModel = Constants.initReportModel();
