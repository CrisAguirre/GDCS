...
import { AspiranteConvocatoriaReportModel } from '@app/compartido/modelos/reportes/aspirante-convocatoria-model';
...
@Component({
  selector: 'app-reporte-aspirante-inscrito',
  templateUrl: './reporte-aspirante-inscrito.component.html',
  styleUrls: ['./reporte-aspirante-inscrito.component.scss']
})
export class ReporteAspiranteInscritoComponent extends BaseController implements OnInit, AfterViewChecked {
  ...
  public async loadData() {
    ...
    let config: any;
    if (this.reportName === '1') {
      config = <any>(<any>await this.commonService.getMessageByName(configMsg.ESTADO_ASPIRANTE_INSCRITO).toPromise()).datos;
    } else if (this.reportName === '2') {
      config = <any>(<any>await this.commonService.getMessageByName(configMsg.ESTADO_ASPIRANTE_ADMITIDO).toPromise()).datos;
    } else if (this.reportName === '3') {
      config = <any>(<any>await this.commonService.getMessageByName(configMsg.ESTADO_ASPIRANTE_NO_ADMITIDO).toPromise()).datos;
    } else {
      this.idEstado = '';
    }

    if (config !== null || config !== undefined) {
      this.idEstado = config.valor;
    }
  }
  ...
  public formValid(val) {
    if (val && this.form.valid) {
      const dataReport: AspiranteConvocatoriaReportModel = Constants.initReportModel();
      dataReport.idConvocatoria = this.f.idConvocatoria.value;
      dataReport.idEstado = this.idEstado;
