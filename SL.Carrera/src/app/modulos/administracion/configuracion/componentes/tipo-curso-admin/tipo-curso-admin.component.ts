...
import { TipoCurso } from '@app/compartido/modelos/tipo-curso';
...
@Component({
  selector: 'app-tipo-curso-admin',
  templateUrl: './tipo-curso-admin.component.html',
  styleUrls: ['./tipo-curso-admin.component.scss']
})
export class TipoCursoAdminComponent extends BaseController implements OnInit, AfterViewChecked {

  public form: FormGroup;
  public displayedColumns: string[] = ['id', 'descripcionCurso', 'options'];
  ...
  public async loadData() {
    ...
    this.dataSource.data = (<any> await this.convoService.getTipoCurso().toPromise()).datos;
  }
  ...
  public edit(element: TipoCurso) {
    this.scrollTop();
    this.elementCurrent = C.cloneObject(element);
    this.form.patchValue({
      id: element.id,
      courseDescription: element.descripcionCurso
    });
  }
  ...
  public addTipoCurso() {
    ...
    const obj = this.dataSource.data.find((x: TipoCurso) => this.areEquals(x.descripcionCurso, this.f.courseDescription.value));
    if (obj) {
    ...
    const newCourseType: TipoCurso = {
      id: this.f.id.value ? this.f.id.value : undefined,
      descripcionCurso: this.f.courseDescription.value,
    };
    this.convoService.saveTipoCurso(newCourseType).toPromise()
      .then(ok => {
        this.loadData().then(() => {
  ...
  public delete(element: TipoCurso) {
    ...
    this.alertService.comfirmation(this.ct.DELETE_CONFIRMATION, TYPES.INFO)
      .then(ok => {
        if (ok.value) {
          ...
          this.convoService.deleteTipoCurso(element)
            .subscribe(o => {
              this.loadData()
                .then(r => {
