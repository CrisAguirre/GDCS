...
@Component({
  selector: 'app-area-conocimiento-admin',
  templateUrl: './area-conocimiento-admin.component.html',
  styleUrls: ['./area-conocimiento-admin.component.scss']
})
export class AreaConocimientoAdminComponent extends BaseController implements OnInit, AfterViewChecked {

  public displayedColumns: string[] = ['id', 'areaConocimiento', 'areaConocimiento_En', 'options'];
  ...
  public async loadData() {
    ...
    this.dataSource.data = (<any> await this.commonService.getAreaKnowledge().toPromise()).datos;
  }
    ...
    this.form = this.fb.group(
      {
        ...
        areaConocimiento_En: new FormControl('', [Validators.required])
      }
    );
    this.reqCampoIngles = this.commonService.campoInglesRequerido(this.f.areaConocimiento_En);
  }
  ...
  public edit(element: AreaConocimiento) {
    this.elementCurrent = C.cloneObject(element);
    this.scrollTop();
    this.form.patchValue({
      id: element.id,
      knowledgeArea: element.areaConocimiento,
      areaConocimiento_En: element.areaConocimiento_En
    });
  }

  public addknowledgeArea() {
    ...
    const obj = this.dataSource.data.find((x: AreaConocimiento) => this.areEquals(x.areaConocimiento, this.f.knowledgeArea.value));
    if (obj) {
    ...
    const newknowledgeArea: AreaConocimiento = {
      id: this.f.id.value ? this.f.id.value : undefined,
      areaConocimiento: this.f.knowledgeArea.value,
      areaConocimiento_En: this.f.areaConocimiento_En.value,
    };
    this.configurationService.saveKnowledgeArea(newknowledgeArea).toPromise()
      .then(ok => {
        this.loadData().then(() => {
   ...
   public delete(element: AreaConocimiento) {
    ...
    this.alertService.comfirmation(this.ct.DELETE_CONFIRMATION, TYPES.INFO)
      .then(ok => {
        if (ok.value) {
          ...
          this.configurationService.deleteKnowledgeArea(element)
            .subscribe(o => {
              this.loadData()
                .then(r => {
