...
@Component({
  selector: 'app-validar-docs-aspirante',
  templateUrl: './validar-docs-aspirante.component.html',
  styleUrls: ['./validar-docs-aspirante.component.scss']
})
export class ValidarDocsAspiranteComponent extends BaseController implements OnInit, AfterViewChecked {

  private user = this.commonService.getVar(configMsg.USER);
  public displayedColumns: string[] = ['identificacion', 'cargoAspirante', 'gradoCargo', 'dependencia', 'lugar', 'estadoAspirante', 'verificado', 'options'];
  ...
  private idSEstadoSAspM2: Configuration;
  ...
  public idEstadoAspiranteCitacion: Configuration;
  ...
  public async loadDataByConvocatoria(pConvocatoria: any) {
    ...
    if (pConvocatoria.value) {
      ...
      this.modelo = this.dataConvocatory.tipoModelo;
      ...
      if (this.lstTable.length > 0) {        
        ...
        if (this.modelo == Constants.MODELO_1) {
          ...
          //filtrar los estados normales
          const ids: string[] = this.idSEstadoSAspM2.valor.split(',');
          this.lstEstadosAspiranteView = this.lstEstadosAspiranteAll.filter(x => ids.includes(x.id));
         
          //filtrar solo los que estan en esa lista de estados
          //this.lstTable = this.lstTable.filter(r => this.lstEstadosAspiranteView.some(s => this.areEqualsIdGuid(s.id, r.idEstadoAspirante)));
        }
        ...
        this.lstTable = this.lstTable.filter(x => {
          ...
          x.esVerificado = x.verificado ? true : false;

          // si es de reclasificacion se lo omite
          if (x.idSolicitudRecalificacion && x.estadoSolicitudRecalificacion != this.labelNO) {
            return false;
          }
          return true;
        });
  ...
  public async loadData() {
    ...
    this.idSEstadoSAspM2 = (await this.commonService.getVarConfig(configMsg.ESTADOS_ASPIRANTES_VERIFICACION_DOCSM2));
  ...
  public async loadAspirante(element: InscripcionAspiranteModel) {
    ...
    //verificar el modelo
    if (this.modelo == Constants.MODELO_1) {
      //validar si es admitido, no admitido e inscrito para mostrar boton de guardar
      const idEstadosAspGuardar = [this.varEstadoAspiranteNoAdmitido.valor, this.idEstadoAspiranteAdmitido.valor, this.idEstadoAspiranteInscrito.valor].join(',');
      ...
      //validar si el estado es aprobo pruebas para mostrar boton de guardar
      const idEstadosAspGuardar = [this.idEstadoAspiranteAproboPruebas.valor, this.varEstadoAspiranteNoAdmitido.valor].join(',');
      if (idEstadosAspGuardar.toUpperCase().includes(element.idEstadoAspirante.toUpperCase())) {
        //mostrar el boton de guardar
        this.showBotonGuardar = true;
      }
      ...
      //validar si es admitido, no admitido mostrar boton de reporte
      // const idEstadosAspReporte = [this.idEstadoAspiranteAproboPruebas.valor, this.varEstadoAspiranteNoAdmitido.valor].join(',');
      // if (idEstadosAspReporte.toUpperCase().includes(element.idEstadoAspirante.toUpperCase())) {
      //   //mostrar el boton de guardar
      //   this.showBotonReporte = true;
      // }
    ...
    const idEstadosAspReporte = [this.idEstadoAspiranteInscrito.valor, this.idEstadoAspiranteCitacion.valor].join(',');
    if (!idEstadosAspReporte.toUpperCase().includes(element.idEstadoAspirante.toUpperCase())) {
      //mostrar el boton de guardar
      this.showBotonReporte = true;
    }
  ...
  public saveAspirante() {
    let formValid = true;
    if (!this.estadoAspiranteSelect.valid) {
      this.estadoAspiranteSelect.markAsTouched();
      this.estadoAspiranteSelect.updateValueAndValidity();
      formValid = false;
    }
    if (this.esNoAdmitido && !this.estadoNoAdmitidoAspirante.valid) {
      this.estadoNoAdmitidoAspirante.markAsTouched();
      this.estadoNoAdmitidoAspirante.updateValueAndValidity();
      formValid = false;
    }
    if (this.esOtraCategoria && !this.otraCategoria.valid) {
      this.otraCategoria.markAsTouched();
      this.otraCategoria.updateValueAndValidity();
      formValid = false;
    }

    if (!this.esNoAdmitido) {
      ...
      // const docInvalido = this.lstAdjuntos.find(x => x.correcto === false);
      // formValid = docInvalido ? false : true;
      // if (!formValid) {
      //   this.alertService.message(this.ct.DOCUMENTOS_CHECK_INVALIDOS, TYPES.WARNING)
      // }
    }
    ...
    if (formValid) {
      ...
      element.idEstadoAspirante = this.estadoAspiranteSelect.value;
      element.idUsuarioModificacion = this.user.id;

      const aspiranteAdmi: AspiranteAdmintidoModel = {
        idInscripcionAspirante: element.id,
        idCategoriaNoAdmitidos: null,
        idUsuario: element.idUsuario,
        observacion: this.observacionesAspirante.value,
        aspiranteAdmitido: this.esNoAdmitido ? 0 : 1,
        otraCategoria: this.esOtraCategoria ? this.otraCategoria.value : null,
        idUsuarioModificacion: this.user.id,
        id: this.aspiranteAdmitidoTemp ? this.aspiranteAdmitidoTemp.id : undefined
      };

      if (this.esNoAdmitido && this.estadoNoAdmitidoAspirante.value) {
        const lstSelect: CategoriaAdmintidoModel[] = this.estadoNoAdmitidoAspirante.value;
        let idString;
        lstSelect.forEach(t => {
          idString = idString ? idString + ',' + t.id : t.id;
        });
        aspiranteAdmi.idCategoriaNoAdmitidos = idString;
      }
          ...
          r => {
            this.convService.saveAspiranteAdmitido(aspiranteAdmi)
              .subscribe(async r2 => {
                ...
                //si es modelo 2 acutliazar en proceso de seleccion
                if (this.modelo == Constants.MODELO_2 && this.areEqualsIdGuid(element.idEstadoAspirante, this.varEstadoAspiranteNoAdmitido.valor)) {
                  await this.convService.updateEstadoProcesoSeleccion(element.id, element.idConvocatoria)
                    .toPromise()
                    .catch(err => this.alertService.showError(err));
                }
  ...
  public exportarReporte() {
    ...
    this.alertService.loading();
    this.convService.getRequisitosByConvocatory(this.f.idConvocatoria.value)
      .toPromise()
      .then(async (res: any) => {
        const requisitos: RequisitosConvocatoriaModel[] = res.datos;
        this.reporteServ.getVerficacionAdmitidos(this.f.idConvocatoria.value, this.lang)
          .toPromise()
          .then((resReporte: Blob) => {
            this.downloadBlob(resReporte, 'reporte');
            if (requisitos.length === 0) {
              const msg: any = JSON.parse(this.varMensajeConv.valor);
              this.alertService.message(this.translateField(msg, 'mensaje', this.lang), TYPES.WARNING)
                .then(() => {
  ...
  public onToggle(event) {
    ...
    if (this.modelo == Constants.MODELO_1) {
      ...
      if (event.checked) {
        this.allPendientes = true;
        this.dataSource.data = this.lstTable.filter(x => !x.verificado);
      } else {
        this.allPendientes = false;
        this.dataSource.data = this.lstTable;
      }
